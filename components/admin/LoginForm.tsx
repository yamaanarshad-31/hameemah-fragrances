...
export function LoginForm() {
  ...
  return (
    <form action={action} className="mt-6 space-y-4">
      {/* React resets the form after the action runs; the email comes back in state so it stays filled in */}
      <label className="block text-xs font-semibold uppercase tracking-widest text-gold-2/80">Email<input name="email" type="email" required autoComplete="username" defaultValue={state?.email} className={cls} /></label>
