  ...
  return (
    <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
      ...
      <div>
        ...
        <button onClick={buyNow} disabled={v.stock <= 0} className="btn-gold mt-3 flex w-full items-center justify-center gap-2 rounded-full py-4 text-sm font-bold uppercase tracking-[0.2em] disabled:opacity-50">
          <Zap className="size-4" /> Buy it now — Cash on delivery
        </button>
        <a href={`https://wa.me/${whatsapp}?text=${encodeURIComponent(`Hi! I want to order ${p.name} (${v.size}).`)}`} target="_blank" rel="noopener noreferrer" className="mt-3 block text-center text-sm font-semibold text-emerald underline-offset-4 hover:underline">
        ...
        <div className="mt-8 grid grid-cols-3 gap-3 text-center text-xs">
          {[[Wallet, "Cash on delivery"], [Truck, "2–4 day delivery"], [ShieldCheck, "Easy exchange"]].map(([I, t]) => {
            ...
            return <div key={t as string} className="rounded-2xl bg-cream-2/60 p-3"><Icon className="mx-auto size-5 text-gold-3" /><p className="mt-1.5 font-semibold">{t as string}</p></div>;
          })}
        </div>
