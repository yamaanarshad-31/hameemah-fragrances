...
export function ReviewForm({ productId }: { productId: number }) {
  ...
  return (
    <form action={action} className="space-y-4 rounded-3xl border border-ink/10 bg-white p-5 sm:p-6">
      <p className="font-display text-2xl">Write a review</p>
      <input type="hidden" name="productId" value={productId} />
      <input type="hidden" name="rating" value={rating} />
      <div className="-mx-1.5 flex" onMouseLeave={() => setHover(0)} role="radiogroup" aria-label="Rating">
        {[1, 2, 3, 4, 5].map((k) => (
          <button type="button" key={k} role="radio" aria-checked={rating === k} aria-label={`${k} star${k > 1 ? "s" : ""}`} onMouseEnter={() => setHover(k)} onClick={() => setRating(k)} className="p-1.5">
            <Star className={`size-7 transition ${k <= (hover || rating) ? "fill-gold text-gold" : "text-ink/20"}`} />
          </button>
        ))}
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        <input name="name" required minLength={2} maxLength={60} placeholder="Your name" aria-label="Your name" className="min-w-0 rounded-xl border border-ink/15 px-4 py-3 focus:border-emerald focus:outline-none" />
        <input name="city" maxLength={40} placeholder="City (optional)" aria-label="City" className="min-w-0 rounded-xl border border-ink/15 px-4 py-3 focus:border-emerald focus:outline-none" />
      </div>
