...
export function ProductTabs({ tabs }: { tabs: { key: string; label: string; items: CardProduct[] }[] }) {
    ...
    <section className="mx-auto max-w-7xl px-5 pb-24 lg:px-8 lg:pb-32">
      <div className="flex flex-col items-center gap-6 sm:flex-row sm:justify-between">
        ...
        <div role="tablist" className="relative flex rounded-full border border-cream-2 bg-white p-1 shadow-sm">
          {tabs.map((t) => (
            <button key={t.key} role="tab" aria-selected={t.key === active} onClick={() => setActive(t.key)} className={`relative z-10 rounded-full px-4 py-2 text-xs font-bold uppercase tracking-[0.14em] transition-colors sm:px-5 ${t.key === active ? "text-cream" : "text-ink/60 hover:text-ink"}`}>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={tab.key} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.35 }} className="mt-12 grid grid-cols-2 gap-x-4 gap-y-10 lg:grid-cols-4 lg:gap-x-6">
          {tab.items.slice(0, 8).map((p, i) => <ProductCard key={p.id} p={p} index={i} />)}
        </motion.div>
      </AnimatePresence>
