...
export function Faq() {
  ...
  return (
    <section className="cv-auto mx-auto max-w-4xl px-5 py-20 sm:py-24 lg:py-32">
      ...
      <div className="mt-12 divide-y divide-ink/10 border-y border-ink/10">
        {FAQS.map((f, i) => (
          <div key={i}>
            <button onClick={() => setOpen(open === i ? null : i)} aria-expanded={open === i} className="flex w-full items-center justify-between gap-4 py-5 text-left sm:gap-6 sm:py-6">
              <span className="font-display text-[1.35rem] leading-snug text-ink sm:text-2xl">{f.q}</span>
