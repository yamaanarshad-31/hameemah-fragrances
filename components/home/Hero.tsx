...
export function Hero({ items, title, subtitle, freeOver }: { items: HeroItem[]; title: string; subtitle: string; freeOver: number }) {
  ...
  return (
    ...
    >
      {/* colour wash follows the perfume on show */}
      <motion.div className="absolute right-[-10%] top-[5%] size-[70vmax] rounded-full opacity-50 blur-[120px] lg:right-[5%] lg:size-[55vmax]" animate={{ backgroundColor: cur?.color ?? "#d4af37" }} transition={{ duration: 1.6 }} />
      <div className="absolute bottom-[-30%] left-[-20%] size-[70vmax] rounded-full bg-emerald/70 blur-[120px]" />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(212,175,55,.12),transparent_55%)]" />
      <GoldDust density={90} />
      {/* light rays */}
      <div className="pointer-events-none absolute right-[8%] top-0 h-full w-[45%] opacity-40 [mask-image:linear-gradient(to_bottom,black,transparent_85%)]">
        {[0, 1, 2].map((k) => (
          <motion.span key={k} className="absolute top-[-10%] h-[120%] w-24 origin-top bg-gradient-to-b from-gold-2/40 to-transparent blur-2xl" style={{ left: `${20 + k * 25}%` }} animate={{ rotate: [k * 6 - 8, k * 6 - 2, k * 6 - 8], opacity: [0.3, 0.7, 0.3] }} transition={{ duration: 7 + k * 2, repeat: Infinity, ease: "easeInOut" }} />
        ))}
      </div>
        ...
        {cur && (
          <motion.div style={{ y: yBottle }} className="relative mx-auto aspect-square w-full max-w-[34rem]">
           <div style={{ ["--i" as string]: 0.2 }} className="hero-pop absolute inset-0">
            ...
            <div className="absolute -left-2 top-1/2 flex -translate-y-1/2 flex-col gap-2 sm:left-0">
              {items.map((it, k) => (
      ...
      <motion.div style={{ opacity: fade }} className="absolute bottom-6 left-1/2 hidden -translate-x-1/2 flex-col items-center gap-2 text-[0.6rem] uppercase tracking-[0.4em] text-cream/40 sm:flex">
        Scroll
        <span className="relative h-10 w-px overflow-hidden bg-cream/15"><motion.span className="absolute inset-x-0 top-0 h-4 bg-gold" animate={{ y: [-16, 40] }} transition={{ duration: 1.6, repeat: Infinity, ease: "easeInOut" }} /></span>
      </motion.div>
    </section>
  );
}
