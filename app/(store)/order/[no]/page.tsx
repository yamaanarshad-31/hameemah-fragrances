...
export default async function OrderPage({ params, searchParams }: PageProps<"/order/[no]">) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-12 sm:py-14 lg:py-20">
      {isNew && <Confetti />}
      <div className="text-center">
        ...
        <h1 className="mt-2 text-balance font-display text-[2.6rem] leading-tight sm:text-6xl">{isNew ? "Your order is placed!" : `Order ${o.orderNo}`}</h1>
      ...
      <div className="mt-10 rounded-[2rem] bg-white p-5 shadow-[0_30px_70px_-40px_rgba(6,20,13,.5)] sm:p-8">
        ...
        <ul className="mt-8 space-y-4 border-t border-ink/10 pt-6">
          {o.items.map((l, i) => (
            <li key={i} className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="min-w-0 flex-1"><p className="font-semibold">{l.name}</p><p className="text-xs text-muted">{l.size} × {l.qty}</p></div>
