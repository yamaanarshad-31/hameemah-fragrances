...
export default async function OrderDetail({ params }: PageProps<"/admin/orders/[id]">) {
  ...
  return (
    <>
      <Link href="/admin/orders" className="mb-4 inline-flex items-center gap-1 text-sm text-muted hover:text-ink print:hidden"><ArrowLeft className="size-4" /> All orders</Link>
      ...
      <div className="grid gap-6 xl:grid-cols-[2fr_1fr]">
          ...
          <ul className="mt-4 divide-y divide-black/5">
            {o.items.map((i, k) => (
              <li key={k} className="flex items-center gap-4 py-3">
                ...
                <div className="flex-1"><p className="font-semibold">{i.name}</p><p className="text-sm text-muted">{i.size} · {rs(i.price)} × {i.qty}</p></div>
                <p className="font-semibold tabular-nums">{rs(i.price * i.qty)}</p>
              </li>
            ))}
          </ul>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <p className="text-sm text-muted">{o.phone}{o.email ? ` · ${o.email}` : ""}</p>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2 print:hidden">
              <a href={`https://wa.me/${wa}?text=${encodeURIComponent(msg)}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 rounded-xl bg-[#25D366] py-2.5 text-sm font-semibold text-white"><MessageCircle className="size-4" /> WhatsApp</a>
              <a href={`tel:${o.phone}`} className="flex items-center justify-center gap-2 rounded-xl bg-ink py-2.5 text-sm font-semibold text-cream"><Phone className="size-4" /> Call</a>
            </div>
          </Card>
